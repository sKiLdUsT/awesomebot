...
class Bot {
  constructor (client, guild) {
    this.client = client
    this.guild = guild
    this.settings = new Settings(this.guild.id, this)
    this.permissions = new Permissions(this.guild.id)
    this.db = new DB(this.guild.id)
    this.guild.members.forEach(member => {
      if (member.id !== this.client.id && !this.permissions[member.id]) {
        this.permissions[member.id] = ''
      }
    })
    this.modules = {
